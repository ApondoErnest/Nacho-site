<section class="rounded-lg border border-gray-200 bg-white shadow-sm">
    ...
    <div class="grid gap-4 p-5 md:grid-cols-2">
        <label class="block">
            ...
            <input name="name_en" value="{{ old('name_en', $category->name_en) }}" required class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-nacho-primary focus:ring-nacho-primary">
            <x-input-error :messages="$errors->get('name_en')" class="mt-2" />
        </label>
        <label class="block">
            <span class="text-sm font-semibold text-gray-700">Name FR</span>
            <input name="name_fr" value="{{ old('name_fr', $category->name_fr) }}" required class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-nacho-primary focus:ring-nacho-primary">
            <x-input-error :messages="$errors->get('name_fr')" class="mt-2" />
        </label>
        <label class="block md:col-span-2">
            <span class="text-sm font-semibold text-gray-700">Slug</span>
            <input name="slug" value="{{ old('slug', $category->slug) }}" required class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-nacho-primary focus:ring-nacho-primary">
...
<section class="rounded-lg border border-gray-200 bg-white shadow-sm">
    ...
    <div class="grid gap-4 p-5 md:grid-cols-2">
        <label class="block">
            ...
            <textarea name="description_en" rows="5" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-nacho-primary focus:ring-nacho-primary">{{ old('description_en', $category->description_en) }}</textarea>
        ...
        <label class="block">
            ...
            <textarea name="description_fr" rows="5" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-nacho-primary focus:ring-nacho-primary">{{ old('description_fr', $category->description_fr) }}</textarea>
