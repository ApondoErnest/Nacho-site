@php
    $statusClasses = [
        'pending' => 'bg-orange-50 text-orange-700',
        'confirmed' => 'bg-blue-50 text-blue-700',
        'arrived' => 'bg-indigo-50 text-indigo-700',
        'in_inspection' => 'bg-purple-50 text-purple-700',
        'completed' => 'bg-green-50 text-green-700',
        'cancelled' => 'bg-red-50 text-red-700',
        'no_show' => 'bg-red-50 text-red-700',
        'rescheduled' => 'bg-yellow-50 text-yellow-700',
    ];
@endphp

<x-admin-layout title="Bookings">
    <div class="space-y-5">
        @if (session('status'))
            <div class="rounded-md border border-green-200 bg-green-50 px-4 py-3 text-sm font-semibold text-green-800">
                {{ session('status') }}
            </div>
        @endif

        <div>
            <p class="text-sm font-semibold text-gray-500">Review inspection requests and update operational status</p>
            <h2 class="mt-1 text-xl font-bold tracking-normal text-gray-950">Booking management</h2>
        </div>

        <form method="GET" action="{{ route('admin.bookings.index') }}" class="grid gap-3 rounded-lg border border-gray-200 bg-white p-4 shadow-sm lg:grid-cols-[minmax(0,1fr)_12rem_12rem_12rem_10rem_10rem_auto]">
            <label class="block">
                <span class="text-sm font-semibold text-gray-700">Search</span>
                <input
                    type="search"
                    name="search"
                    value="{{ $search }}"
                    placeholder="Reference, name, phone, plate"
                    class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-nacho-primary focus:ring-nacho-primary"
                >
            </label>

            <label class="block">
                <span class="text-sm font-semibold text-gray-700">Status</span>
                <select name="status" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-nacho-primary focus:ring-nacho-primary">
                    <option value="">All statuses</option>
                    @foreach ($statuses as $case)
                        <option value="{{ $case->value }}" @selected($status === $case->value)>{{ str($case->value)->replace('_', ' ')->title() }}</option>
                    @endforeach
                </select>
            </label>

            <label class="block">
                <span class="text-sm font-semibold text-gray-700">Center</span>
                <select name="center_id" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-nacho-primary focus:ring-nacho-primary">
                    <option value="">All centers</option>
                    @foreach ($centers as $center)
                        <option value="{{ $center->id }}" @selected((string) $centerId === (string) $center->id)>{{ $center->name_en }}</option>
                    @endforeach
                </select>
            </label>

            <label class="block">
                <span class="text-sm font-semibold text-gray-700">Service</span>
                <select name="service_id" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-nacho-primary focus:ring-nacho-primary">
                    <option value="">All services</option>
                    @foreach ($services as $service)
                        <option value="{{ $service->id }}" @selected((string) $serviceId === (string) $service->id)>{{ $service->title_en }}</option>
                    @endforeach
                </select>
            </label>

            <label class="block">
                <span class="text-sm font-semibold text-gray-700">From</span>
                <input type="date" name="date_from" value="{{ $dateFrom }}" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-nacho-primary focus:ring-nacho-primary">
            </label>

            <label class="block">
                <span class="text-sm font-semibold text-gray-700">To</span>
                <input type="date" name="date_to" value="{{ $dateTo }}" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-nacho-primary focus:ring-nacho-primary">
            </label>

            <div class="flex items-end gap-2">
                <button type="submit" class="inline-flex min-h-10 items-center justify-center gap-2 rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-bold text-gray-800 hover:bg-gray-50">
                    <x-lucide-search class="h-4 w-4" aria-hidden="true" />
                    <span>Filter</span>
                </button>
                <a href="{{ route('admin.bookings.index') }}" class="inline-flex min-h-10 items-center justify-center rounded-md border border-gray-200 px-3 py-2 text-sm font-bold text-gray-600 hover:bg-gray-50">
                    Reset
                </a>
            </div>
        </form>

        <div class="grid gap-3 lg:hidden">
            @forelse ($bookings as $booking)
                <article class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0">
                            <p class="break-all text-xs font-bold uppercase text-nacho-primary">{{ $booking->booking_reference }}</p>
                            <h3 class="mt-1 break-words text-sm font-bold text-gray-950">{{ $booking->full_name }}</h3>
                            <p class="mt-1 text-xs font-semibold text-gray-500">{{ $booking->phone }}</p>
                        </div>
                        <span @class([
                            'shrink-0 rounded-full px-2.5 py-1 text-xs font-bold',
                            $statusClasses[$booking->status->value] ?? 'bg-gray-100 text-gray-600',
                        ])>
                            {{ str($booking->status->value)->replace('_', ' ')->title() }}
                        </span>
                    </div>

                    <dl class="mt-4 grid grid-cols-2 gap-2 text-xs">
                        <div class="rounded-md bg-gray-50 px-2 py-2">
                            <dt class="font-semibold text-gray-500">Visit</dt>
                            <dd class="mt-1 font-bold text-gray-950">{{ $booking->preferred_date?->format('M j, Y') }} · {{ $booking->preferred_time }}</dd>
                        </div>
                        <div class="rounded-md bg-gray-50 px-2 py-2">
                            <dt class="font-semibold text-gray-500">Vehicle</dt>
                            <dd class="mt-1 font-bold text-gray-950">{{ $booking->vehicle_registration }}</dd>
                        </div>
                        <div class="rounded-md bg-gray-50 px-2 py-2">
                            <dt class="font-semibold text-gray-500">Center</dt>
                            <dd class="mt-1 font-bold text-gray-950">{{ $booking->center?->name_en ?: 'Not set' }}</dd>
                        </div>
                        <div class="rounded-md bg-gray-50 px-2 py-2">
                            <dt class="font-semibold text-gray-500">Service</dt>
                            <dd class="mt-1 font-bold text-gray-950">{{ $booking->service?->title_en ?: 'Not set' }}</dd>
                        </div>
                    </dl>

                    <div class="mt-4 flex justify-end">
                        <a href="{{ route('admin.bookings.show', $booking) }}" class="inline-flex min-h-10 items-center justify-center gap-2 rounded-md border border-gray-200 px-3 py-2 text-xs font-bold text-gray-700 hover:bg-gray-50">
                            <x-lucide-eye class="h-4 w-4" aria-hidden="true" />
                            <span>View</span>
                        </a>
                    </div>
                </article>
            @empty
                <div class="rounded-lg border border-gray-200 bg-white px-4 py-10 text-center text-sm font-semibold text-gray-500 shadow-sm">
                    No bookings match the current filters.
                </div>
            @endforelse
        </div>

        <div class="hidden overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm lg:block">
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    <thead class="bg-gray-50 text-left text-xs font-bold uppercase tracking-wide text-gray-500">
                        <tr>
                            <th scope="col" class="px-4 py-3">Booking</th>
                            <th scope="col" class="px-4 py-3">Visit</th>
                            <th scope="col" class="px-4 py-3">Center</th>
                            <th scope="col" class="px-4 py-3">Service</th>
                            <th scope="col" class="px-4 py-3">Vehicle</th>
                            <th scope="col" class="px-4 py-3">Status</th>
                            <th scope="col" class="px-4 py-3"><span class="sr-only">Actions</span></th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($bookings as $booking)
                            <tr class="align-top">
                                <td class="px-4 py-4">
                                    <div class="font-bold text-gray-950">{{ $booking->booking_reference }}</div>
                                    <div class="mt-1 text-gray-500">{{ $booking->full_name }}</div>
                                    <div class="mt-1 text-xs font-semibold text-gray-500">{{ $booking->phone }}</div>
                                </td>
                                <td class="px-4 py-4 text-gray-700">{{ $booking->preferred_date?->format('M j, Y') }}<br>{{ $booking->preferred_time }}</td>
                                <td class="px-4 py-4 text-gray-700">{{ $booking->center?->name_en ?: 'Not set' }}</td>
                                <td class="px-4 py-4 text-gray-700">{{ $booking->service?->title_en ?: 'Not set' }}</td>
                                <td class="px-4 py-4 font-bold text-gray-950">{{ $booking->vehicle_registration }}</td>
                                <td class="px-4 py-4">
                                    <span @class([
                                        'inline-flex rounded-full px-2.5 py-1 text-xs font-bold',
                                        $statusClasses[$booking->status->value] ?? 'bg-gray-100 text-gray-600',
                                    ])>
                                        {{ str($booking->status->value)->replace('_', ' ')->title() }}
                                    </span>
                                </td>
                                <td class="px-4 py-4">
                                    <div class="flex justify-end">
                                        <a href="{{ route('admin.bookings.show', $booking) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-md border border-gray-200 text-gray-600 hover:bg-gray-50 hover:text-gray-950">
                                            <span class="sr-only">View {{ $booking->booking_reference }}</span>
                                            <x-lucide-eye class="h-4 w-4" aria-hidden="true" />
                                        </a>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="px-4 py-10 text-center text-sm font-semibold text-gray-500">
                                    No bookings match the current filters.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        </div>

        {{ $bookings->links() }}
    </div>
</x-admin-layout>
